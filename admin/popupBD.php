...
$pasta = "../arquivos/layout/popups/";     
$sucesso = "<script>alert('Operação realizada com sucesso');</script><meta http-equiv='refresh' content='0;URL=home.php?page=Popups'> ";   
$erro = "<script>alert('Ocorreu algum erro. Tente novamente!');</script><meta http-equiv='refresh' content='0;URL=home.php?page=Popups'> ";  
?> 

   <?php
 if (isset($_POST['documento'])){
 $cod = $_POST['documento'];
 $sqlb = mysqli_query($link,"SELECT * FROM popups where cod=$cod") or die("ERRO NO SQL");
...
<?php
                      ...
                      $foto = $_FILES['foto'];
                        ...
                        if(isset($_FILES['foto']) && $_FILES['foto']['size'] > 0):   
                            ...
                            $sql = mysqli_query($link,"INSERT INTO popups (`cod`, `nome`,`foto`) VALUES 
                            ('','$nome','$nomeFoto')") or die ("Houve erro na gravação dos dados" . mysqli_error()); 
                            ...
                            if($sql) {
                              ...
                              echo $erro;
                                ...
                                if ($alteraFoto == "sim"):
                                  ...
                                  $sql = mysqli_query($link,"UPDATE popups SET nome='$nome',  status=$status, foto='$nomeFoto' WHERE cod=$cod ") or die ("Houve erro na gravação dos dados" . mysqli_error());              
                                else:
                    $sql = mysqli_query($link,"UPDATE popups SET nome='$nome', status=$status WHERE cod=$cod") or die ("Houve erro na gravação dos dados" . mysqli_error()); 
                endif;          
                if($sql) {
                  echo $sucesso; 
                } else {
                echo $erro;
                      ...
                      $sqlb = mysqli_query($link,"SELECT foto FROM popups where cod=$cod") or die("ERRO NO SQL");
                      $dados = mysqli_fetch_array($sqlb);
                                         $foto = $dados['foto'];
                                         unlink($pasta.$foto);                 
                    $sqld = mysqli_query($link,"DELETE FROM popups where cod=$cod") or die("ERRO NO SQL");
                    if($sqld) {
                      echo $sucesso; 
                    } else {
                    echo $erro;
